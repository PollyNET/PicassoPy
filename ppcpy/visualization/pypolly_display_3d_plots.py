...
def downsample_nearest(arr: np.ndarray, factor: int) -> np.ndarray:
    ...
    - `arr`  : 2‑D‑Array (dtype, e.g. float64)
    - `factor`: integer-value Down‑Sampling‑Faktor (> 1)

    returns a 2-d-array, which is downsampled by N-th factor row and column
    """
    if factor <= 1:
        return arr.copy()

    ## padding if height/width can not be divided by factor
    h, w = arr.shape
    pad_h = (factor - (h % factor)) % factor
    pad_w = (factor - (w % factor)) % factor
    if pad_h or pad_w:
        # border pixel repeating
        arr = np.pad(arr, ((0, pad_h), (0, pad_w)), mode='edge')

    # factorize N-th element
...
def pollyDisplayAttnBsc(nc_dict, config_dict, polly_conf_dict, saveFolder, wavelength, param,donefilelist_dict):
    ...
    ## read from nc file
    ATT_BETA = nc_dict[f'attenuated_backscatter_{wavelength}nm']['data']
    if param == 'FR' or param == 'NR':
        if f'SNR_{wavelength}nm' in nc_dict.keys():
            SNR = nc_dict[f'SNR_{wavelength}nm']['data']
        quality_mask = nc_dict[f'quality_mask_{wavelength}nm']['data']
    elif param == 'OC':
...
#    retrieving_info = "{\"Lidar_calibration_constant_used\": {\"value\": 17741303565181.887, \"unit\": \"\"}}"
    #print(nc_dict[f'attenuated_backscatter_{wavelength}nm___retrieving_info']['Lidar_calibration_constant_used'])
    #retrieving_info = json.loads(nc_dict[f'attenuated_backscatter_{wavelength}nm___retrieving_info'])
    retrieving_info = json.loads(nc_dict[f'attenuated_backscatter_{wavelength}nm']['attributes']['retrieving_info'])
    LCUsed = retrieving_info['Lidar_calibration_constant_used']['value']
    ...
    #print(nc_dict['m_date'])
    date_00 = datetime.strptime(nc_dict['m_date'], '%Y%m%d')
    #date_00 = datetime.strptime(nc_dict['m_date'], '%Y-%m-%d')
    #date_00 = datetime.strptime(nc_dict['m_date'], '%Y%m%d')
    date_00 = date_00.timestamp()
    #date_00 = nc_dict['m_date'].timestamp()
    
    ## set x-lim to 24h or only to last available timestamp
    x_lims = readout.set_x_lims(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],mdate=date_00,last_timestamp=nc_dict['time']['data'][-1])
    ...
    ATT_BETA = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=ATT_BETA,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
...
#    cbar.ax.set_title('      $Mm^{-1}*sr^{-1}$\n', fontsize=10)
    ...
    readout.write2donefilelist_dict(donefilelist_dict = donefilelist_dict,
                                    lidar = pollyVersion,
                                    location = nc_dict['location'],
                                    starttime = datetime.utcfromtimestamp(int(nc_dict['time']['data'][0])).strftime('%Y%m%d %H:%M:%S'),
                                    stoptime = datetime.utcfromtimestamp(int(nc_dict['time']['data'][-1])).strftime('%Y%m%d %H:%M:%S'),
                                    ...
                                    active = 1,
                                    GDAS = 0,
                                    GDAS_timestamp = f"{datetime.utcfromtimestamp(int(nc_dict['time']['data'][0])).strftime('%Y%m%d')} 12:00:00",
                                    lidar_ratio = 50,
                                    software_version = version,
                                    product_type = f'ATT_BETA_{wavelength}{prod_type}',
                                    product_starttime = datetime.utcfromtimestamp(int(nc_dict['time']['data'][0])).strftime('%Y%m%d %H:%M:%S'),
                                    product_stoptime = datetime.utcfromtimestamp(int(nc_dict['time']['data'][-1])).strftime('%Y%m%d %H:%M:%S')
                                    )
    ...
    if param == 'FR' or param == 'NR':
        ...
        ## trimm matrix to last available timestamp if neccessary
        SNR = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=SNR,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
        ...
        SNR = np.flip(SNR,0)
        SNR = downsample_nearest(SNR, factor=4)
        #SNR[SNR == 0] = np.nan
        #SNR[SNR < zLim[0]] = np.nan
        SNR[SNR < 0] = np.nan
   ...
   #            datenum_to_datetime(time[0]).strftime("%Y-%m-%d"),
        ...
        plt.close()
        remove_widht_height_entry_from_svg_file(saveFilename_SNR,imgFormat)
...
def pollyDisplayVDR(nc_dict,config_dict,polly_conf_dict,saveFolder, wavelength,donefilelist_dict):
    ...
    Usage
    -----
    pollyDisplayVDR(nc_dict,config_dict,polly_conf_dict,saveFolder, wavelength)
    ...
    VDR = nc_dict[f'volume_depolarization_ratio_{wavelength}nm']['data']
    quality_mask = np.where(VDR > 0, 0, 0)
    #eta = re.split(r'eta:',nc_dict[f'volume_depolarization_ratio_{wavelength}nm___comment'])[1]
    #retrieving_info = json.loads(nc_dict[f'volume_depolarization_ratio_{wavelength}nm___retrieving_info'])
    retrieving_info = json.loads(nc_dict[f'volume_depolarization_ratio_{wavelength}nm']['attributes']['retrieving_info'])
    
    try:
        eta = float(retrieving_info['eta']['value'])
    except Exception as e:
        eta = '' 
    height = nc_dict['height']['data']
    time = nc_dict['time']['data']
...
#    y_max = 0.3
    ...
    ## trimm matrix to last available timestamp if neccessary
    VDR = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=VDR,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
    ...
    #import copy
    #cmap =copy.copy(plt.cm.get_cmap(colormap_basic))
    VDR = downsample_nearest(VDR, factor=4)
    ## set color of nan-values
    cmap.set_bad(color='white')
...
def pollyDisplayWVMR(nc_dict,config_dict,polly_conf_dict,saveFolder,donefilelist_dict):
    ...
    ## read from nc-file
    WVMR = nc_dict['WVMR']['data']
    SNR387 = nc_dict[f'SNR_387nm']['data']
    SNR407 = nc_dict[f'SNR_407nm']['data']
    quality_mask = nc_dict['QM_WVMR']['data']
    height = nc_dict['height']['data']
    time = nc_dict['time']['data']
    ...
    ## trimm matrix to last available timestamp if neccessary
    WVMR = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=WVMR,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
...
#            datenum_to_datetime(time[0]).strftime("%Y-%m-%d"),
    ...
    plt.close()
    retrieving_info = json.loads(nc_dict['WVMR']['attributes']['retrieving_info'])
    remove_widht_height_entry_from_svg_file(saveFilename,imgFormat)
...
#    ## mask matrix
    ...
    ## trimm matrix to last available timestamp if neccessary
    SNR387 = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=SNR387,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
    SNR407 = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=SNR407,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
...
#            datenum_to_datetime(time[0]).strftime("%Y-%m-%d"),
    ...
    plt.close()
    remove_widht_height_entry_from_svg_file(saveFilename_SNR387,imgFormat)
...
#            datenum_to_datetime(time[0]).strftime("%Y-%m-%d"),
    ...
    plt.close()
    remove_widht_height_entry_from_svg_file(saveFilename_SNR407,imgFormat)
...
def pollyDisplayRH(nc_dict,config_dict,polly_conf_dict,saveFolder,donefilelist_dict):
    ...
    ## trimm matrix to last available timestamp if neccessary
    RH = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=RH,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
...
def pollyDisplayTargetClass(nc_dict,config_dict, polly_conf_dict, saveFolder, c_version, donefilelist_dict):
    ...
    matrix = nc_dict['target_classification']['data']
    quality_mask = np.where(matrix > 0, 0, 0)
    height = nc_dict['height']['data']
    time = nc_dict['time']['data']
    #cRange = nc_dict['target_classification']['attributes']['plot_range'] ## equals zLim and the number of classes in the target classification
    cRange = [0,len(json.loads(nc_dict['target_classification']['attributes']['definition']))-1]

    TC_def = json.loads(nc_dict['target_classification']['attributes']['definition'])

    #classes = re.split(r'\\n',TC_def)
    classes_list = []
    for c_key in TC_def.keys():
        classes_list.append(TC_def[c_key])
   # for i in range(int(cRange[1])+1):
   #     T_class = re.split(r'[0-9]: ', classes[i])[1] 
   #     classes_list.append(re.split(r'\\', T_class)[0])
    legends = json.loads(nc_dict['target_classification']['attributes']['target_classification_legend_keys'])


    TC_legend_key_red = legends['legend_key_red']
    TC_legend_key_green = legends['legend_key_green']
    TC_legend_key_blue = legends['legend_key_blue']
...
#    saveFolder = args.outdir
    ...
    ## trimm matrix to last available timestamp if neccessary
    matrix = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=matrix,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])
    ## transpose and flip for correct plotting
...
#    cmap = load_colormap(name=colormap_basic)
    ...
    matrix= downsample_nearest(matrix, factor=4)
    cmap.set_bad(color='white')
...
def pollyDisplay_preprocessed(nc_dict, config_dict, polly_conf_dict, saveFolder, variable,donefilelist_dict):
    """
    Description
    -----------
    ...
    Parameters
    ----------
    variable: variables for SNR or RCS or BG, e.g. RCS_532_total_NR
    nc_dict: dict
        dict which stores the preprocessed data (SNR,RCS or BG).

    Usage
    -----
    pollyDisplay_preprocessed(nc_dict, config_dict, polly_conf_dict, saveFolder, variable,donefilelist_dict)
    ...
    var, wavelength, polarization, telescope = readout.gf(variable)
    #print(var, wavelength, polarization, telescope)
    ## read from global config file
#    yLim = polly_conf_dict[f'yLim_{telescope}_{var}']
    yLim = polly_conf_dict[f'yLim_{telescope}_RCS']
    #zLim = polly_conf_dict[f'zLim_{telescope}_{var}_{wavelength}']
    try:
        zLim = polly_conf_dict[f'zLim_{telescope}_{var}_{wavelength}']
    except:
        zLim = [None,None]
    ...
    ## read from nc file
    data_matrix = nc_dict[variable]['data']
    quality_mask = np.where(data_matrix > 0, 0, -999.0)
    ...
    location = nc_dict['location']
    version = nc_dict['PicassoVersion']
    dataFilename = re.split(f'_{var}',nc_dict['PollyDataFile'])[0]

    # set the default font
    matplotlib.rcParams['font.sans-serif'] = fontname
    matplotlib.rcParams['font.family'] = "sans-serif"

    plotfile = f'{dataFilename}_{variable}.{imgFormat}'
    saveFilename = os.path.join(saveFolder,plotfile)

    ## fill time gaps in att_bsc matrix
    data_matrix = readout.fill_time_gaps_of_single_matrix(time, data_matrix)
...
#    ## mask matrix
    data_matrix = np.ma.masked_where(data_matrix <= 0, data_matrix)
    
    ## slice matrix to max_height
    data_matrix = data_matrix[:,0:len(max_height)]

    ## trimm matrix to last available timestamp if neccessary
    data_matrix = readout.trimm_matrix_to_last_timestamp(flagPlotLastProfilesOnly=config_dict['flagPlotLastProfilesOnly'],matrix=data_matrix,mdate=date_00,profile_length=int(np.nanmean(np.diff(time))),last_timestamp=nc_dict['time']['data'][-1])

    ## transpose and flip for correct plotting
    data_matrix= np.ma.transpose(data_matrix)  ## matrix has to be transposed for usage with pcolormesh!
    data_matrix= np.flip(data_matrix,0)
...
#    
    ...
    ax.set_title(
        '{var} at {wave} nm'.format(var=var,wave = wavelength) +
        ' {param} of {instrument} at {location}'.format(
            param=telescope,
            instrument=pollyVersion,
            location=location),
    ...
    if zLim[0] is None or zLim[1] is None:
        ...
        tick_values = np.linspace(zLim[0], zLim[1], 5)
    cb_ax = fig.add_axes([0.92, 0.25, 0.02, 0.55])
    cbar = fig.colorbar(
        pcmesh,
        cax=cb_ax,
...
#    cbar.ax.set_title('      $Mm^{-1}*sr^{-1}$\n', fontsize=10)
    ...
    readout.write2donefilelist_dict(donefilelist_dict = donefilelist_dict,
                                    ...
                                    filename = saveFilename,
                                    level = 0,
                                    info = f"{var} plots for {telescope} at {wavelength}",
                                    nc_zip_file = nc_dict['PollyDataFile'],
                                    nc_zip_file_size = 9000000,
                                    active = 1,
                                    GDAS = 0,
...
def pollyDisplay_preprocessed_BG(nc_dict, config_dict, polly_conf_dict, saveFolder, variable,donefilelist_dict):
    """
    Description
    -----------
    Display the RangeCorrected (FR/NR) of wavelength [nm] channel from level1 polly nc-file.
    ...
    timediffs = np.diff(time)
    diffs_rounded = np.round(timediffs, decimals=3)
    vals, counts = np.unique(diffs_rounded, return_counts=True)
    common_delta = vals[np.argmax(counts)]
    seconds_in_day = 86400

    # 00:00:00 UTC dieses Tages
    start_of_day = (time[0] // seconds_in_day) * seconds_in_day
    
    # Bis exakt zum Ende des Tages (00:00:00 UTC des Folgetages)
    end_of_day = start_of_day + seconds_in_day
    
    # --- SCHRITT 3: Den kompletten Tag lückenlos generieren ---
    # np.arange stoppt EXKLUSIVE dem Endwert. 
    # Bei 30s Delta ist der letzte Punkt somit 23:59:30 UTC.
    perfect_day_timestamps = np.arange(start_of_day, end_of_day, common_delta)
    ...
    ## fill time gaps in att_bsc matrix
    data_matrix_h = np.expand_dims(data_matrix, axis=1) ## add dummy-height-dim
    data_matrix_h = readout.fill_time_gaps_of_single_matrix(time, data_matrix_h)
    ...
    fig = plt.figure(figsize=[12, 6])
    ax = fig.add_axes([0.11, 0.15, 0.79, 0.75])
    ax.plot(perfect_day_timestamps.astype('datetime64[s]'),data_matrix)
    # convert the datetime data from a float (which is the output of date2num into a nice datetime string.
    ax.xaxis_date()

    ax.set_xlabel('Time [UTC]', fontsize=15)
    ax.set_ylabel('Background [a.u.]', fontsize=15)
...
#    
    ...
    ax.set_title(
        'Background Signal at {wave} nm'.format(wave = wavelength) +
        ' {param} of {instrument} at {location}'.format(
            param=telescope,
            instrument=pollyVersion,
            location=location),
