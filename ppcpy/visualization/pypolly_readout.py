...
def get_nc_filename(date, device, inputfolder, param=""):
    ...
    param: str
        ...
        "NR_profiles", "cloudinfo","POLIPHON_1" ,"POLIPHON_2", "RCS"
    '''
...
def read_nc_file(nc_filename,timestamp,device,location):
    ...
    for v_count,var_name in enumerate(var_ls):
        nc_dict[var_name] = {
                            "data": nc_file_ds[var_name][:],
                            "attributes": {}
                            }
        for var_att in nc_file_ds.variables[var_name].ncattrs():
            var_att_value = nc_file_ds.variables[var_name].getncattr(var_att)
            nc_dict[var_name]['attributes'][var_att] = var_att_value
...
def extract_wavelength_from_parameterkey(parameter):
    """
    get wv from parameter, e.g.: aerBsc_klett_355 --> returns 355

    """

    # Find the last underscore
    last_underscore_index = parameter.rfind('_')
    
    # Extract everything after the last underscore
    after_last_underscore = parameter[last_underscore_index + 1:]
    
    # Check if it's a 3-4 digit number
    if after_last_underscore.isdigit() and 3 <= len(after_last_underscore) <= 4:
        result = after_last_underscore
    else:
        result = None
    return result
