...
def lc_for_cldFreeGrps(data_cube, retrieval:str, collect_debug=False) -> list:
    ...
    - Through the config variable 'flagUseRetrievedExtForLC', the extinction used to calculate the LCs can be specified. if 
      ...
      the backscatter times the assumed lidar constant will be used. In the matlab version the retrieved extinction is used 
      for the ... and the approximated extinction is used for ....
    ...
    **History**
    ...
    for i, cldFree in enumerate(data_cube.clFreeGrps):
        ...
        for channel in profiles:
            ...
            if config_dict['flagUseRetrievedExtForLC']:
            ...
            if config_dict['flagPicassoComparison']:
                if 'klett' in retrieval:
                    aerExt = profiles[channel]['aerExt']
                    aerExt[:hBaseInd + 1] = aerExt[hBaseInd]
                elif 'raman' in retrieval:
                    aerBsc[aerBsc <= 0] = np.nan
                    aerExt = aerBsc * config_dict[f'{key_LR}{wv}']
                else:
