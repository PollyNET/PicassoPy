...
def overlapCalc(height, sigFR, bgFR, sigNR, bgNR, hFullOverlap=600):
    ...
    ret = {'overlap': overlap, 'overlapStd': overlapStd, 
...
def overlapCalcRaman(Lambda_el, Lambda_Ra, height, sigFRel, sigFRRa, 
    ...
    if len(aerBsc) > 0:
        ...
        for ii in range(len(LR0) + 1):
            ...
            ovl_norm, _, _ = mean_stable(olFunc, 40, fullOverlapIndx - round(37.5 / hres), fullOverlapIndx + round(2250 / hres), 0.1)
            ovl_norm0, _, _ = mean_stable(olFunc0, 40, fullOverlapIndx - round(37.5 / hres), fullOverlapIndx + round(2250 / hres), 0.1)
    ...
    ret = {'overlap': olFunc, 'overlap_raw': olFunc0}
    return ret
