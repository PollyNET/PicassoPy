...
picasso_config_file = "H:\\Pollynet_Processing_Chain\\config\\pollynet_processing_chain_config_rsd2_24h_exp.json"
...
polly_config_array = loadConfigs.readPollyNetConfigLinkTable(picasso_config_dict['pollynet_config_link_file'],timestamp=20240803,device="pollyxt_cpv")
polly_config_file = str(polly_config_array['Config file'].to_string(index=False)).strip() ## get rid of whtiespaces
...
## add campaign_infos to polly_config_dict
polly_config_dict['name'] = str(polly_config_array['Instrument'].to_string(index=False)).strip()
polly_config_dict['location'] = str(polly_config_array['Location'].to_string(index=False)).strip()
polly_config_dict['startTime'] = str(polly_config_array['Campaign Starttime'].to_string(index=False)).strip()
polly_config_dict['endTime'] = str(polly_config_array['Campaign Stoptime'].to_string(index=False)).strip()
polly_config_dict['lon'] = str(polly_config_array['Longitude'].to_string(index=False)).strip()
polly_config_dict['lat'] = str(polly_config_array['Latitude'].to_string(index=False)).strip()
polly_config_dict['asl'] = str(polly_config_array['asl.'].to_string(index=False)).strip()


#for k in polly_config_dict:
#    print(f'{k}: {polly_config_dict[k]}')
